...
// dp[i][j] indicates number of ways coin change can be done using denomications till "i" and till value "j"
// for every cell two choices either include and exclude (total ways = include + exclude)
...
int countWaysToMakeChange(int denominations[], int numDenominations, int value){
    ...
    for(int i=1;i<=n;i++){
        for(int j=1;j<=w;j++){
    		if(j >= denominations[i-1]){  //(value is greater than denominations ,then try including)
...
// int helper(int arr[],int n,int value,int** dp){
...
// 	int ans =  helper(arr+1,n-1,value,dp) + helper(arr,n,value-arr[0],dp);
// 	dp[n][value] = ans;
//     return ans;
// }
