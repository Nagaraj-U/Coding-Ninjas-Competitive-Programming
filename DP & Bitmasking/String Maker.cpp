...
ll helper(string a,string b,string c,ll dp[55][55][55])
{
    ...
    for(int i=0;i<a.length();i++){
        if(a[i] == c[0]){
            count += (helper(a.substr(i+1),b,c.substr(1),dp))%mod;
        }
    }
    
    for(int i=0;i<b.length();i++){
        if(b[i] == c[0]){
            count += (helper(a,b.substr(i+1),c.substr(1),dp))%mod;
        }
    }
    dp[a.length()][b.length()][c.length()] = count%mod;
    return count%mod;
}
