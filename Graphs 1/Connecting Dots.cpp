...
/*
bool getans(char board[][MAXN],int n,int m,int i,int j,char temp,bool** visited){
    
    int count = 1;
    if(i-1 >= 0 && !visited[i-1][j] && board[i-1][j] == temp){
        visited[i-1][j] = true;
        count++;
        if(getans(board,n,m,i-1,j,temp,visited)){
           return true
        }else{
            visited[i-1][j] = false;
        }
    ...
    if(j-1 >= 0 && !visited[i][j-1] && board[i][j-1] == temp){
        visited[i][j-1] = true;
        count++;
        
    	if(getans(board,n,m,i,j-1,temp,visited)){
        	if(count >= 4){
                return true;
            }
        }else{
    ...
    if(j+1 < m && !visited[i][j+1] && board[i][j+1] == temp){
        visited[i][j+1] = true;
        count++;
    ...
    if(i+1 < n && !visited[i+1][j] && board[i+1][j] == temp){
        visited[i+1][j] = true;
        count++;
...
int solve(char board[][MAXN],int n, int m)
{
    ...
    for(int i=0;i<n;i++){
        for(int j=0;j<m;j++){
            char temp = board[i][j];
            // visited[i][j] = true;
            bool ans  = getans(board,n,m,i,j,temp,visited);
            if(ans){
                return 1;
            }
            for(int p=0;p<n;p++){
